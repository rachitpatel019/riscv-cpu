...
*   **Power Consumption:** 350.90 mW total power (97.33 mW static, 244.62 mW dynamic, 8.95 mW I/O)
...
| **Static Power** | **97.33 mW** | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |
| **Dynamic Power** | **244.62 mW** (Core dynamic) | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |
| **I/O Power** | **8.95 mW** | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |
| **Total Power** | **350.90 mW** | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |

### Memory Read Enable Gating Power Optimization
To optimize dynamic power dissipation in the data memory module ([data_mem.sv](../rtl/core/7_ex3_mem/data_mem.sv)), the write tracking registers (`write_addr_reg`, `write_data_reg`, etc.) used for Read-During-Write (RDW) hazard bypass logic were wrapped with an explicit read-enable condition (`if (mem_read)`). This ensures write tracking state is sampled on clock edges only when an active memory read operation is requested, preventing redundant register toggling during non-read execution cycles. This optimization reduced dynamic core power dissipation by **3.57 mW** (from 248.19 mW down to 244.62 mW, bringing total power to 350.90 mW) while reducing logic utilization to **2,895 LEs**.
...
| :--- | :--- | :--- |
| **Energy Per Instruction (EPI)** | **4.215 nJ/instruction** | $(\text{Total Power} \times \text{Execution Time}) / \text{Instructions Retired}$ |
| **Performance Per Watt** | **2.3725 × 10⁸ inst/J** | $\text{Instructions Retired} / \text{Total Energy}$ |
| **MIPS/Watt** | **237.25 MIPS/W** | $\text{MIPS} / \text{Total Power (W)}$ |
