...
The 8-Stage Pipeline RISC-V CPU is a high-performance, single-issue, in-order processor implementing the RV32I base integer instruction set. Designed specifically for FPGA deployment, the architecture divides the standard 5-stage RISC-V pipeline into 8 stages. This optimization reduces the combinational logic depth per stage, yielding an operating clock frequency of **125.00 MHz** (via `pll_125` with +0.110 ns setup slack margin) on the Intel MAX 10 FPGA.

Key metrics of the hardware and workload evaluation include:
*   **Clock Frequency:** 125.00 MHz ($T_{\text{clk}} = 8.000\text{ ns}$, generated by `pll_125`)
*   **Performance:** 83.25 MIPS (at 0.6660 IPC / 1.5015 CPI)
*   **Power Consumption:** 275.34 mW total power (96.97 mW static, 169.41 mW dynamic, 8.95 mW I/O)
*   **FPGA Logic Utilization:** 2,895 / 49,760 Logic Elements (6%)
...
| :--- | :--- | :--- |
| **Clock Frequency** | **125.00 MHz** ($T_{\text{clk}} = 8.000\text{ ns}$, Slack: **+0.110 ns**) | Clock constraint in [cpu.sdc](../fpga/cpu.sdc), generated by `pll_125` |
| **Logic Utilization** | **2,895 / 49,760 LEs** (6%) | [cpu.fit.summary](../fpga/output_files/cpu.fit.summary) |
...
| **Static Power** | **96.97 mW** | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |
| **Dynamic Power** | **169.41 mW** (Core dynamic) | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |
| **I/O Power** | **8.95 mW** | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |
| **Total Power** | **275.34 mW** | [cpu.pow.rpt](../fpga/output_files/cpu.pow.rpt) |

### Memory Read Enable Gating Power Optimization
To optimize dynamic power dissipation in the data memory module ([data_mem.sv](../rtl/core/7_ex3_mem/data_mem.sv)), the write tracking registers (`write_addr_reg`, `write_data_reg`, etc.) used for Read-During-Write (RDW) hazard bypass logic were wrapped with an explicit read-enable condition (`if (mem_read)`). This ensures write tracking state is sampled on clock edges only when an active memory read operation is requested, preventing redundant register toggling during non-read execution cycles. Combined with VCD simulation activity parsing (`power.vcd`), core dynamic power dissipation is **169.41 mW** and core static power is **96.97 mW**, yielding a total power dissipation of **275.34 mW** while maintaining low logic utilization at **2,895 LEs**.
...
| **8-Stage Pipeline (Active)** | **125.00 MHz** | **2,895** | **1,303** | **64 KiB (16,384 words)** | **64 KiB (16,384 words)** |
...
| Processor Name | Max Clock Frequency ($F_{\text{max}}$) | Resource Utilization (MAX 10 Logic Elements) | Estimated Performance (MIPS / DMIPS) | Reliable Source of Information / Documentation Link |
| :--- | :--- | :--- | :--- | :--- |
| **8-Stage Pipeline CPU (Active)** | **125.00 MHz** | **2,895 LEs** | **83.25 MIPS** | [Section 1 (Physical Synthesis)](#1-physical-synthesis--power-metrics) & [Section 3 (Simulation)](#3-simulation--workload-performance) |
...
| :--- | :--- | :--- | :--- |
| **Stage 1** | Fetch Logic | 6.682 ns | Includes branch feedback path from Stage 7 |
| **Stage 2** | I-Mem Read | 5.929 ns | BRAM synchronous read latency dominates |
| **Stage 3** | **Decode & Hazard** | **6.925 ns** | **Critical stage: decode combinational logic and hazard detection** |
| **Stage 4** | Register Read | 5.386 ns | Forwarding unit combinational logic |
| **Stage 5** | EX1 (Op Sel) | 5.578 ns | Operand selection mux chain |
| **Stage 6** | EX2 (ALU) | 6.649 ns | ALU arithmetic and logic execution |
| **Stage 7** | EX3 & MEM Addr | 3.982 ns | PC target calculation and memory address |
| **Stage 8** | WB Logic | 5.176 ns | Write-back mux to register file write port |

### Critical Path Analysis
*   **Worst setup path slack:** **+0.110 ns** (Data delay of **7.782 ns**).
*   **Critical Path Destination:** EX1_EX2 pipeline register (`core:cpu_core|EX1_EX2:stage5_ex1_ex2_reg|operand_a_out[30]`).
*   **Interpretation:** The design operates at **125.00 MHz** ($T_{\text{clk}} = 8.000\text{ ns}$) with a positive setup slack margin of **+0.110 ns**. The tightest physical setup path terminates at the Stage 5 to Stage 6 boundary register (`operand_a_out[30]`).
...
| **MIPS (Millions of Inst/Sec)** | **83.25 MIPS** | $125.00\text{ MHz} \times \text{IPC}$ |
...
| :--- | :--- | :--- |
| **Energy Per Instruction (EPI)** | **3.307 nJ/instruction** | $(\text{Total Power} \times \text{Execution Time}) / \text{Instructions Retired}$ |
| **Performance Per Watt** | **3.0234 × 10⁸ inst/J** | $\text{Instructions Retired} / \text{Total Energy}$ |
| **MIPS/Watt** | **302.35 MIPS/W** | $\text{MIPS} / \text{Total Power (W)}$ |
| **Area Efficiency** | **28.76 KIPS/LE** | $\text{MIPS} / \text{Logic Elements}$ (with 2,895 LEs) |
...
1.  **Functional Unit & Integration Testbenches:**
    The design includes 21 dedicated functional testbenches in the [tb/](../tb/) directory to validate the logic of individual sub-modules and core integrations. These unit-level verification tests cover:
    *   *Unit-level blocks:* ALU, BHT, Branch Eval, Branch Predictor, Control Unit, Data Selector, Decoder, Forwarding Unit, Forwarding Selector (`fwd_sel`), Pipeline Control Unit, Immediate Generator, Instruction Memory, Data Memory, PC Update, PC Target Calculator, Regfile, and Writeback logic.
    ...
    *   *Regression Runner:* The PowerShell script [run_all.ps1](../sim/scripts/run_all.ps1) automates compiling and running all 18 unit testbenches sequentially in ModelSim/QuestaSim and prints a PASS/FAIL summary table (18/18 passing).
