...
logic [$clog2(MEM_DEPTH)-1:0] masked_addr;
assign masked_addr = ((address - 32'h80020000) >> 2) & (MEM_DEPTH-1);

// Synchronous write port logic handling byte, halfword, and word masking.
always_ff @(posedge clk) begin
    if (mem_write) begin
        case (mem_size)
            2'b10: memory[masked_addr] <= write_data;
            2'b01:
                case (address[1])
                    1'b1: memory[masked_addr][31:16] <= write_data[15:0];
                    1'b0: memory[masked_addr][15:0] <= write_data[15:0];
                endcase
            2'b00:
                case (address[1:0])
                    2'b11: memory[masked_addr][31:24] <= write_data[7:0];
                    2'b10: memory[masked_addr][23:16] <= write_data[7:0];
                    2'b01: memory[masked_addr][15:8] <= write_data[7:0];
                    2'b00: memory[masked_addr][7:0] <= write_data[7:0];
                endcase
            default: memory[masked_addr] <= write_data;
        endcase
    end
end
...
always_ff @(posedge clk) begin
    current_word <= memory[masked_addr];
