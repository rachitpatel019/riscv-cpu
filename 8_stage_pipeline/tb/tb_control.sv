...
module tb_control;
    ...
    task check_control(
        ...
        if (uses_rs1 === i_uses_rs1 && uses_rs2 === i_uses_rs2 && alu_op === i_alu_op && 
            ...
            tests_failed++;
            $error("MISMATCH: Time=%t, Instr=%h, Actual: uses_rs1=%b, uses_rs2=%b, alu_op=%b, alu_src_a=%b, alu_src_b=%b, reg_write=%b, mem_read=%b, mem_write=%b, mem_size=%b, mem_unsigned=%b, wb_sel=%b, branch=%b, jump=%b, branch_type=%b", 
                   $time, instruction, uses_rs1, uses_rs2, alu_op, alu_src_a, alu_src_b, reg_write, mem_read, mem_write, mem_size, mem_unsigned, wb_sel, branch, jump, branch_type);
        end
        tests_total++;
    endtask
    ...
    initial begin
        ...
        check_control(1, 1, ALU_ADD, 0, 0, 1, 0, 0, 2'b00, 0, 2'b00, 0, 0, 3'b000);

        // I-Type (ADDI x1, x0, 10) -> 00a00093
        drive_control(32'h00a00093);
        check_control(1, 0, ALU_ADD, 0, 1, 1, 0, 0, 2'b00, 0, 2'b00, 0, 0, 3'b000);

        // LW x1, 4(x2) -> 00412083
        drive_control(32'h00412083);
        check_control(1, 0, ALU_ADD, 0, 1, 1, 1, 0, 2'b10, 0, 2'b01, 0, 0, 3'b010);

        // SW x2, 4(x1) -> 0020a223
        drive_control(32'h0020a223);
        check_control(1, 1, ALU_ADD, 0, 1, 0, 0, 1, 2'b10, 0, 2'b00, 0, 0, 3'b010);

        // BEQ x1, x2, 8 -> 00208463
        drive_control(32'h00208463);
        check_control(1, 1, ALU_SUB, 0, 0, 0, 0, 0, 2'b00, 0, 2'b00, 1, 0, 3'b000);

        // JAL x1, 4 -> 004000ef
        drive_control(32'h004000ef);
        check_control(0, 0, ALU_ADD, 1, 1, 1, 0, 0, 2'b00, 0, 2'b10, 0, 1, 3'b000);

        // LUI x1, 0x1000 -> 010000b7
        drive_control(32'h010000b7);
        check_control(0, 0, ALU_PASS, 0, 1, 1, 0, 0, 2'b00, 0, 2'b00, 0, 0, 3'b000);

        // AUIPC x1, 0x1000 -> 01000097
        drive_control(32'h01000097);
        check_control(0, 0, ALU_ADD, 1, 1, 1, 0, 0, 2'b00, 0, 2'b00, 0, 0, 3'b000);

        // Invalid Opcode
        drive_control(32'h00000000);
        check_control(0, 0, ALU_ADD, 0, 0, 0, 0, 0, 2'b00, 0, 2'b00, 0, 0, 3'b000);
