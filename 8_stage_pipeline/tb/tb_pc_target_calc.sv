...
task automatic check(input logic exp_sel, input logic [31:0] exp_target);
    if (pc_sel === exp_sel && pc_target === exp_target) begin
        ...
        report_error("CHECK", $sformatf("MISMATCH: ExpSel=%b, ActSel=%b, ExpTarget=%h, ActTarget=%h", 
            exp_sel, pc_sel, exp_target, pc_target));
    end
endtask
...
initial begin
    ...
    drive(32'h100, 32'h5, 32'h5, 1, 0, 3'b000, 32'h8, 0); check(1, 32'h108);
    drive(32'h100, 32'h5, 32'h6, 1, 0, 3'b000, 32'h8, 0); check(0, 32'h0);

    drive(32'h100, 0, 0, 0, 1, 0, 0, 32'h200); check(1, 32'h200);

    drive(32'h100, 32'h5, 32'h5, 1, 1, 3'b000, 32'h8, 32'h200); check(1, 32'h200);

    drive(32'h100, 32'h5, 32'h5, 1, 0, 3'b000, 32'hFFFFFFF8, 0); check(1, 32'hF8);
